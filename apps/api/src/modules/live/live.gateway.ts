...
export class LiveGateway {
 ...
 publishLocation(tripId:string,payload:unknown){this.server.to(`trip:${tripId}`).emit('trip.location.updated',payload);}
 ...
 publishEnded(tripId:string){this.server.to(`trip:${tripId}`).emit('trip.completed',{tripId});}
}
