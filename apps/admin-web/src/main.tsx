...
import { SUGAT_BRAND_TAGLINE } from '@sugat/theme';
...
type Tab = 'dashboard' | 'drivers' | 'verification' | 'vehicles' | 'stops' | 'routes' | 'schedules' | 'live';
type RecordData = Record<string, any>;
type Data = { dashboard: RecordData; drivers: RecordData[]; verification: RecordData[]; vehicles: RecordData[]; stops: RecordData[]; routes: RecordData[]; schedules: RecordData[]; live: RecordData[] };
...
async function request(path: string, init: RequestInit = {}) {
  const send = () => fetch(`${API}${path}`, { ...init, headers: { ...(init.body instanceof FormData ? {} : { 'content-type': 'application/json' }), authorization: `Bearer ${accessToken}`, ...init.headers } });
...
function App() {
  ...
  async function load(quiet = false) {
    ...
    try {
      const [dashboard, drivers, verification, vehicles, stops, routes, schedules, live] = await Promise.all([
        '/admin/dashboard', '/admin/drivers', '/admin/verification/submissions', '/admin/vehicles', '/admin/stops', '/admin/routes', '/admin/schedules', '/admin/live',
      ].map(path => request(path)));
      setData({ dashboard, drivers, verification, vehicles, stops, routes, schedules, live }); setError('');
    } catch (caught) {
  ...
  async function login(event: FormEvent) {
    ...
    try { const response = await fetch(`${API}/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: email.trim().toLowerCase(), password }) }); if (!response.ok) throw new Error(await messageFrom(response)); storeTokens(await response.json()); setLoggedIn(true); }
  ...
  if (!loggedIn) return <main className="login"><form onSubmit={login}><img className="official-logo login-logo" src="/sugat-logo-official.png" alt="SUGAT" /><p className="kicker">{SUGAT_BRAND_TAGLINE}</p><h1>Operations login</h1><label>Email<input type="email" value={email} onChange={event => setEmail(event.target.value)} required /></label><label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} required /></label>{error && <p className="alert error">{error}</p>}<button className="primary" disabled={loading}>{loading ? 'SIGNING IN…' : 'SIGN IN'}</button></form></main>;

  return <div className="app">
    <aside><div><img className="official-logo sidebar-logo" src="/sugat-logo-official.png" alt="SUGAT" /><p>{SUGAT_BRAND_TAGLINE}</p><p>ADMIN OPERATIONS</p></div><nav>{(['dashboard', 'drivers', 'verification', 'vehicles', 'stops', 'routes', 'schedules', 'live'] as Tab[]).map(item => <button className={tab === item ? 'active' : ''} onClick={() => { setTab(item); setError(''); }} key={item}>{item.toUpperCase()}</button>)}</nav><button onClick={() => { clearTokens(); setLoggedIn(false); }}>LOG OUT</button></aside>
    <main className="content"><header><div><p className="kicker">{SUGAT_BRAND_TAGLINE}</p><h1>{tab === 'schedules' ? 'SCHEDULES / TRIPS' : tab.toUpperCase()}</h1></div><button className="secondary" onClick={() => load()} disabled={loading}>{loading ? 'REFRESHING…' : 'REFRESH DATA'}</button></header>
      {error && <p className="alert error">{error}</p>}{success && <p className="alert success">{success}</p>}
      {!data ? <Loading /> : <Page tab={tab} data={data} open={setModal} mutate={mutate} navigate={setTab} />}
    </main>
...
function Page({ tab, data, open, mutate, navigate }: { tab: Tab; data: Data; open: (value: any) => void; mutate: Function; navigate: (tab: Tab) => void }) {
  if (tab === 'dashboard') return <Dashboard values={data.dashboard} navigate={navigate} />;
  if (tab === 'drivers') return <Drivers rows={data.drivers} open={open} mutate={mutate} />;
  if (tab === 'verification') return <Verification submissions={data.verification} drivers={data.drivers} mutate={mutate} />;
...
function Verification({submissions,drivers,mutate}:{submissions:RecordData[];drivers:RecordData[];mutate:Function}){
 const[driverId,setDriverId]=useState(''),[files,setFiles]=useState<RecordData>({}),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 async function upload(event:FormEvent){event.preventDefault();if(!driverId||!files.licenseFront||!files.licenseBack||!files.selfie)return;const body=new FormData();body.append('licenseFront',files.licenseFront);body.append('licenseBack',files.licenseBack);body.append('selfie',files.selfie);setBusy(true);setMessage('');try{await request(`/admin/verification/drivers/${driverId}/submissions`,{method:'POST',body});setMessage('Evidence submitted for protected review. Refresh data to see the new submission.');setDriverId('');setFiles({})}catch(caught){setMessage(caught instanceof Error?caught.message:'Upload failed.')}finally{setBusy(false)}}
 async function view(id:string){try{const response=await fetch(`${API}/admin/verification/documents/${id}`,{headers:{authorization:`Bearer ${accessToken}`}});if(!response.ok)throw new Error(await messageFrom(response));const url=URL.createObjectURL(await response.blob());window.open(url,'_blank','noopener,noreferrer');window.setTimeout(()=>URL.revokeObjectURL(url),60_000)}catch(caught){setMessage(caught instanceof Error?caught.message:'Document could not be opened.')}}
 return <><form className="record" onSubmit={upload}><div><h3>SUBMIT DRIVER EVIDENCE</h3><p>All three private documents are required. JPEG, PNG, or PDF; maximum 5 MB each.</p><select value={driverId} onChange={event=>setDriverId(event.target.value)} required><option value="">Select driver…</option>{drivers.map(driver=><option key={driver.id} value={driver.id}>{driverName(driver)}</option>)}</select><label>License front<input type="file" accept="image/jpeg,image/png,application/pdf" onChange={event=>setFiles((value:RecordData)=>({...value,licenseFront:event.target.files?.[0]}))} required/></label><label>License back<input type="file" accept="image/jpeg,image/png,application/pdf" onChange={event=>setFiles((value:RecordData)=>({...value,licenseBack:event.target.files?.[0]}))} required/></label><label>Driver selfie<input type="file" accept="image/jpeg,image/png,application/pdf" onChange={event=>setFiles((value:RecordData)=>({...value,selfie:event.target.files?.[0]}))} required/></label>{message&&<p className="alert">{message}</p>}</div><button className="primary" disabled={busy}>{busy?'UPLOADING…':'SUBMIT EVIDENCE'}</button></form><div className="cards">{submissions.map(item=><article className="record" key={item.id}><div><span className={`status ${item.status==='APPROVED'?'good':item.status==='REJECTED'?'muted':'warn'}`}>{item.status}</span><h3>{driverName(item.driver)}</h3><p>Submitted {localDate(item.submittedAt)}</p>{item.rejectionReason&&<p className="alert error">{item.rejectionReason}</p>}<div className="actions">{item.documents.map((document:RecordData)=><button type="button" key={document.id} onClick={()=>void view(document.id)}>VIEW {document.type.replace('_',' ')}</button>)}</div></div>{['PENDING_VERIFICATION','PENDING_REVERIFICATION'].includes(item.status)&&<div className="actions"><button className="primary" onClick={()=>mutate(`/admin/verification/submissions/${item.id}/review`,'POST',{decision:'APPROVED'},'Verification approved.')}>APPROVE</button><button className="danger" onClick={()=>{const reason=prompt('Rejection reason (required)');if(reason?.trim())mutate(`/admin/verification/submissions/${item.id}/review`,'POST',{decision:'REJECTED',reason:reason.trim()},'Verification rejected.')}}>REJECT</button></div>}</article>)}</div></>;
}
function Dashboard({ values, navigate }: { values: RecordData; navigate: (tab: Tab) => void }) {
  const labels: Record<string, string> = { activeTrips: 'Active Trips', activeBuses: 'Active Buses', activeVans: 'Active Vans', driversOnActiveTrips: 'Drivers on Active Trips', gpsStale: 'GPS Stale', completedToday: 'Completed Today', licensesExpiringWithin30Days: 'Licenses ≤30 Days', licensesExpiringWithin7Days: 'Licenses ≤7 Days', expiredLicenses: 'Expired Licenses', driversPendingVerification: 'Pending Verification' };
  return <><section className="operations-hero"><div><img className="official-logo hero-logo" src="/sugat-logo-official.png" alt="SUGAT" /><p className="kicker">{SUGAT_BRAND_TAGLINE}</p><p className="kicker">REAL-TIME TRANSPORT CONTROL</p><h2>SUGAT OPERATIONS</h2><p>Monitor trips, drivers, vehicles, routes, and live transport activity.</p><button className="hero-cta" onClick={() => navigate('live')}>VIEW LIVE OPERATIONS</button></div><div className="hero-summary"><span><strong>{values.activeTrips ?? '—'}</strong> ACTIVE TRIPS</span><span><strong>{values.gpsStale ?? '—'}</strong> GPS STALE</span></div><div className="hero-route" /></section><section className="metrics">{Object.entries(labels).map(([key, label]) => <article key={key}><span>{label}</span><strong>{values[key] ?? '—'}</strong></article>)}</section></>;
}
...
function Modal({ spec, data, busy, close, mutate }: { spec: { kind: string; item?: RecordData }; data: Data; busy: boolean; close: () => void; mutate: Function }) {
  if (spec.kind === 'viewDriver') return <div className="overlay" onMouseDown={event => event.target === event.currentTarget && close()}><section className="modal"><div className="modal-head"><div><p className="kicker">DRIVER PROFILE</p><h2>{driverName(spec.item)}</h2></div><button onClick={close}>✕</button></div><dl><dt>Email</dt><dd>{spec.item?.user.email}</dd><dt>Mobile</dt><dd>{spec.item?.user.phone || '—'}</dd><dt>Account</dt><dd>{spec.item?.user.accountStatus}</dd><dt>Identity verification</dt><dd>{spec.item?.identityVerificationStatus}</dd><dt>License verification</dt><dd>{spec.item?.licenseVerificationStatus}</dd><dt>License validity</dt><dd>{spec.item?.compliance.licenseValidity}{spec.item?.compliance.licenseDaysRemaining !== null ? ` · ${spec.item?.compliance.licenseDaysRemaining} day(s)` : ''}</dd><dt>Eligibility</dt><dd>{spec.item?.compliance.eligible ? 'ELIGIBLE' : `${spec.item?.compliance.code} — ${spec.item?.compliance.message}`}</dd><dt>License</dt><dd>{spec.item?.licenseNumber || '—'}</dd><dt>License expiration</dt><dd>{spec.item?.licenseExpiresAt ? new Date(spec.item.licenseExpiresAt).toLocaleDateString() : '—'}</dd><dt>Assigned vehicles</dt><dd>{spec.item?.vehicles.map((vehicle: RecordData) => vehicle.displayName).join(', ') || 'None'}</dd></dl></section></div>;
